...
describe('api specific - asset-hub update-asset-label warning visibility', () => {
  beforeEach(() => {
    vi.clearAllMocks()
      ...
      appearances: [
        {
          ...
          changeReason: '默认形象',
          imageUrls: null,
          imageUrl: 'cos://global-image-1',
        },
      ],
    })
  })

  it('returns structured warning when global character image label update fails', async () => {
    ...
    expect(body.labelWarnings[0]).toEqual(expect.objectContaining({
      ...
      target: 'globalCharacterAppearance:appearance-1:image:0',
    }))
    expect(body.labelWarnings[0].detail).toContain('无法归一化媒体 key')
    expect(body.results[0]?.imageUrls?.[0]).toBe('cos://global-image-1')
    expect(prismaMock.globalCharacterAppearance.update).toHaveBeenCalledTimes(1)
  })
})
